...
def main():
    ...
    if args.load != None: 
        if os.path.isfile(args.load):
            ...
            body_test = BodyPoseSet(img_transform=img_trans, label_transform = label_trans)
            test1_loader = DataLoader(body_test, batch_size=configs['batch_train'], shuffle=True, num_workers=configs['num_workers'])
            total_loss = 0.0
            total_correct = 0

            # first do a single image test to see if training works
            sample = body_test[100]
            img = sample['img']
            y = sample['label']
            y_pred = net(img)
